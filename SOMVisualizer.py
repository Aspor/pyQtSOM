...
class SOMVisualizer(QWidget):
    ...
    def addDataLabels(self, data,labels):
        ...
        self.addDataLabelsWorker(data,labels)

    def addDataLabelsWorker(self,data,labels):
         """Add labels to corresponding nodes"""
         self.removeLabels()
         data,dataMin,dataMax=som.normalize(data)
         w=self.w
         h=self.h
         for i,dat in enumerate(data):
             nearest=self.som.nearestNeigbor(data[i],True)
             x= w*nearest[1]*0.75
